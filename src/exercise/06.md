...
- must be class components
  - Use `static getDerivedStateFromError()` to render fallback UI
  - Use `componentDidCatch()` to log errors
- can't catch errors on:
  - event handlers (don't happen during render cycle, so it doesn't actually
    break the UI)
  - async code
  - SSR
  - errors within the ErrorBoundary component

**Extra credit 7**

- the `key` prop helps React literally unmount and remount the component when it
  changes. React keeps track of internal state by comparing the key prop of the
  various elements in the UI, and if that element with the same key prop isn't
  found, React thinks it's just gone; similarly, when an element with a
  different key prop appears for the first time, React assumes it's new and
  mounts it with default state. So if the key prop changes across renders, it
  will effectively get re-rendered completely.
